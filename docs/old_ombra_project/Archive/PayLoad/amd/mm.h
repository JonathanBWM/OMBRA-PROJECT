// PayLoad/amd/mm.h
// AMD SVM Memory Management Interface
...
//===----------------------------------------------------------------------===//
// NPT (Nested Page Table) Entries - AMD Specific
//===----------------------------------------------------------------------===//
...
} npt_pml4e, *pnpt_pml4e, npt_pdpte, *pnpt_pdpte, npt_pde, *pnpt_pde;
...
//===----------------------------------------------------------------------===//
// Self-referencing PML4 for host page table access
//===----------------------------------------------------------------------===//
...
// Initialize identity mapping for physical memory access
auto init() -> u64;
...
// Map physical page to host virtual
auto map_page(host_phys_t phys_addr, map_type_t map_type = map_type_t::map_src) -> u64;

// Translate host virtual to host physical using self-ref PML4
auto translate(host_virt_t host_virt) -> u64;

// Get NPT PTE for a guest physical address
auto get_npte(guest_phys_t phys_addr) -> pnpt_pte;

// Translate guest physical through NPT to get system physical
