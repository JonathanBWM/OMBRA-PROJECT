...
// NTSTATUS codes not always defined in user mode headers
...
// Global kernel image variables - use inline to avoid ODR violations
inline DWORD_PTR g_KernelAddr = 0;
inline PVOID g_KernelImage = nullptr;

inline bool MatchSign(PUCHAR Data, PUCHAR Sign, int Size)
{
...
inline ULONG64 GetKernelObject(ULONG TargetProcessId, HANDLE TargetHandle)
{
    NTSTATUS Status = 0;
    ULONG64 Result = 0;
...
inline PVOID GetSystemInformation(SYSTEM_INFORMATION_CLASS InfoClass)
{
    NTSTATUS Status = 0;
...
inline DWORD GetThreadState(DWORD dwProcessId, DWORD dwThreadId)
{
    DWORD Ret = -1;
...
inline PVOID GetObjectAddress(HANDLE hObject)
{
    PVOID Ret = NULL;
...
inline PVOID GetKernelModule(const char* DriverName)
{
...
inline DWORD64 LeakGadgetAddress(LPCSTR GadgetName)
{
    DWORD64 module_base_kernel = 0, rtlSetAllBits_address = 0;
    HMODULE module_base_user = NULL;
    ...
    // FIX: Free the loaded library to prevent memory leak
    FreeLibrary(module_base_user);

    return rtlSetAllBits_address;
}

inline void* AllocateBitmap(SIZE_T size, LPVOID baseAddress) {
...
inline bool GetKernelImageInfo(PVOID* pImageAddress, PDWORD pdwImageSize, char* lpszName)
{
...
inline bool GetSyscallNumber(const char* lpszProcName, PDWORD pdwRet)
{
...
inline DWORD LdrGetProcAddress(PVOID Image, const char* lpszName)
{
...
inline bool LdrProcessRelocs(PVOID Image, PVOID NewBase)
{
    ...
    while (RelocSize > Size && pRelocation->SizeOfBlock)
    {
        ...
        // FIX: Save current block size BEFORE moving to next block
        DWORD CurrentBlockSize = pRelocation->SizeOfBlock;
        pRelocation = (PIMAGE_BASE_RELOCATION)RVATOVA(pRelocation, CurrentBlockSize);
...
inline bool LdrMapImage(PVOID Data, DWORD dwDataSize, PVOID* pImage, PDWORD pdwImageSize)
{
...
inline bool ReadFromFile(HANDLE hFile, PVOID* pData, PDWORD pdwDataSize)
{
    bool bRet = FALSE;
...
inline bool ReadFromFile(LPCSTR lpszFileName, PVOID* pData, PDWORD pdwDataSize)
{
    bool bRet = false;
...
inline void* GetKernelProcAddress(const char* procName)
{
    if (!g_KernelImage || !g_KernelAddr) {
        return nullptr;
    }

    DWORD offset = LdrGetProcAddress(g_KernelImage, procName);
    if (offset) {
        return RVATOVA(g_KernelAddr, offset);
    }

    return nullptr;
}

inline void* GetKernelZwProcAddress(const char* procName)
{
    void* addr = nullptr;
