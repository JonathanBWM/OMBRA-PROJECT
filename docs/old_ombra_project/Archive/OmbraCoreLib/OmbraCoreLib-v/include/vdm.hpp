...
public:
	...
	OmbraVdm(ULONG64 ombraKey, ULONG64 callbackAddress, ULONG64 cr3 = 0) {
		_ombraKey = ombraKey;
		...
		if (!callbackAddress) {
			NTSTATUS ntStatus = CPU::CPUIDVmCall(0x9898, (ULONG64)&_callbackAddress, 0, _ombraKey);
			if (ntStatus != 0)
	...
	__forceinline void Init(ULONG64 ombraKey, ULONG64 callbackAddress, ULONG64 cr3 = 0) {
		_ombraKey = ombraKey;
		_callbackAddress = callbackAddress;
		_ntoskrnlAddress = GetKernelModuleAddress("ntoskrnl.exe");
		_gameCr3 = cr3;
	...
	__forceinline BOOLEAN ReadMemory(ULONG64 Source, PVOID Destination, SIZE_T NumberOfBytes, ULONG64 cr3 = ~0ull)
	{
		...
		NTSTATUS ntStatus = CPU::CPUIDVmCall(VMCALL_READ_VIRT, (ULONG64)readData, cr3 != ~0ull ? cr3 : _gameCr3, _ombraKey);
		if (ntStatus)
	...
	__forceinline BOOLEAN WriteMemory(ULONG64 Destination, PVOID Source, SIZE_T NumberOfBytes, ULONG64 cr3 = ~0ull)
	{
		...
		NTSTATUS ntStatus = CPU::CPUIDVmCall(VMCALL_WRITE_VIRT, (ULONG64)writeData, cr3 != ~0ull ? cr3 : _gameCr3, _ombraKey);
		if (ntStatus)
