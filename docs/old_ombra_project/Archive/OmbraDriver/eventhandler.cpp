...
NTSTATUS InsertEventHandler(ULONG64& ulOpt1, ULONG64& ulOpt2, ULONG64& ulOpt3) {
	...
	OMBRA_EVENT_TYPE evt = (OMBRA_EVENT_TYPE)ulOpt1;
	switch (evt) {
	case EVT_DETECTION:
	{
		EVENT_INFO evtInfo;
		...
		evtInfo.type = evt;
		OmbraEvent* pEvt = (OmbraEvent*)paging::vmmhost::MapGuestToHost(vmm::GetGuestCR3().Flags, (PVOID)ulOpt2);
...
void eventhandler::OnDetection()
{
	...
	for (auto& evt : *vEvents) {
		...
		OmbraEvent* pEvt = (OmbraEvent*)paging::vmmhost::MapGuestToHost(evt.guestCr3, evt.guestAddr);
		pEvt->Trigger();
	}
}
