...
PVOID GetCallback() {
	PVOID Callback = 0;
	CPU::CPUIdVmcall(VMCALL_GET_CALLBACK, (ULONG64)&Callback, 0, _ombraKey);
	return Callback;
}
```

Then, using a VDM-like interface (you should use OmbraVdm inside vdm.h) you can hijack a syscall to communicate with the driver using 
...
Assuming all next functions to be inside a class caching the callback address and ombra key:
