...
PspInsertThread(
	...
	if (NT_SUCCESS(ntStatus)) {
		...
		PPEB_OMBRA CurrentPEB = (PPEB_OMBRA)PsGetProcessPeb(pEprocess);
...
NtLockVirtualMemory (
	...
	if (CurrentProcess() == ombra::storage_get<PEPROCESS>(VMX_ROOT_STORAGE::CURRENT_CONTROLLER_PROCESS) && kReq->IsValid()) {
...
NTSTATUS comms::Entry(KERNEL_REQUEST* pKernelRequest)
{
	...
	if (OmbraLib::pUserInfo->cleanupData.pDriverName[0]) {
		if (!defender::CleanFilterList(OmbraLib::pUserInfo->cleanupData.pDriverName)) {
			DbgMsg("[CLEANUP] Defender FilterList could not be cleared!");
		}
		OmbraLib::pUserInfo->cleanupData.pDriverName[0] = 0;
	}
