...
UINT64 ombra::VMEXIT_KEY = 0;

void ombra::set_vmcall_key(u64 key)
{
...
auto ombra::current_dirbase()->guest_phys_t
{
...
auto ombra::root_dirbase() -> guest_phys_t
{
    COMMAND_DATA command = { 0 };
...
auto ombra::current_ept_base() -> guest_phys_t
{
    COMMAND_DATA command = { 0 };
...
VMX_ROOT_ERROR ombra::set_ept_base(guest_phys_t nCr3) {
    COMMAND_DATA command = { 0 };
...
void ombra::set_ept_handler(guest_virt_t handler) {
    storage_set(EPT_HANDLER_ADDRESS, handler);
}

VMX_ROOT_ERROR ombra::disable_ept()
{
    return hypercall(VMCALL_TYPE::VMCALL_DISABLE_EPT, 0, 0, VMEXIT_KEY);
}

VMX_ROOT_ERROR ombra::enable_ept()
{
    return hypercall(VMCALL_TYPE::VMCALL_ENABLE_EPT, 0, 0, VMEXIT_KEY);
}

auto ombra::read_phys(guest_phys_t phys_addr, guest_virt_t buffer, u64 size) -> VMX_ROOT_ERROR
{
    COMMAND_DATA command = { 0 };
...
auto ombra::write_phys(guest_phys_t phys_addr, guest_virt_t buffer, u64 size) -> VMX_ROOT_ERROR
{
    COMMAND_DATA command = { 0 };
...
auto ombra::read_virt(guest_virt_t virt_addr, guest_virt_t buffer, u64 size, u64 target_cr3) -> VMX_ROOT_ERROR
{
    COMMAND_DATA command = { 0 };
    command.read.length = size;
...
auto ombra::write_virt(guest_virt_t virt_addr, guest_virt_t buffer, u64 size, u64 target_cr3) -> VMX_ROOT_ERROR
{
    COMMAND_DATA command = { 0 };
    command.write.length = size;
    command.write.pInBuf = (PVOID)buffer;
...
auto ombra::virt_to_phy(guest_virt_t p, u64 dirbase) -> guest_phys_t
{
    COMMAND_DATA command = { 0 };
