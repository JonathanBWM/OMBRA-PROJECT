...
int identity::Init(DWORD64 cr3) {
	...
	identity::IDENTITY_MAPPING* mapping = (identity::IDENTITY_MAPPING*)ombra::malloc_locked_aligned(sizeof(identity::IDENTITY_MAPPING), 0x1000);
	RtlZeroMemory(mapping, sizeof(*mapping));

	mapping->pml4[0].layout.P = true;
	mapping->pml4[0].layout.RW = true;
	mapping->pml4[0].layout.US = true;
	mapping->pml4[0].layout.PDP = ombra::virt_to_phy((UINT64)&mapping->pdpt[0]) / 0x1000;
	...
	for (UINT64 EntryIndex = 0; EntryIndex < 512; EntryIndex++)
	{
		...
		mapping->pdpt[EntryIndex].layout.PD = ombra::virt_to_phy((UINT64)&mapping->pdt[EntryIndex][0]) / 0x1000;
	}
	...
	Pte::Tables<Pte::Mode::longMode4Level>::Pml4e* ppml4 = (Pte::Tables<Pte::Mode::longMode4Level>::Pml4e*)ombra::malloc_locked(0x1000);
	RtlZeroMemory(ppml4, 0x1000);
	if (ombra::read_phys(cr3, (u64)ppml4, 0x1000) != VMX_ROOT_ERROR::SUCCESS) {
		return false;
	}

	ppml4[identity::mapped_host_phys_pml].raw = mapping->pml4[0].raw;
	ombra::write_phys(cr3, (u64)ppml4, 0x1000);
